   ...
   X3DFieldInterface
{
   ...
   {
      stream += wrappedValue .map { String (format: stream .floatFormat, stream .toUnit (unit, value: $0)) } .joined (separator: stream .Comma + stream .TidySpace)
   }
   ...
   internal final override func toJSONStream (_ stream : X3DOutputStream)
   {
      ...
      stream += wrappedValue .map { String (format: stream .floatFormat, stream .toUnit (unit, value: $0)) } .joined (separator: "," + stream .TidySpace)
      stream += stream .TidySpace
      stream += "]"
   }
