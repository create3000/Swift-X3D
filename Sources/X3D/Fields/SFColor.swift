   ...
   X3DFieldInterface
{
   ...
   internal final override func toJSONStream (_ stream : X3DOutputStream)
   {
      ...
      stream += String (format: "\(stream .floatFormat),\(stream .TidySpace)\(stream .floatFormat),\(stream .TidySpace)\(stream .floatFormat)", wrappedValue .r, wrappedValue .g, wrappedValue .b)
      stream += stream .TidySpace
      stream += "]"
   }
