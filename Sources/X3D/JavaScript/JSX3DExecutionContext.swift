...
""")
      ...
      {
         return executionContext .toXMLString (with: executionContext .scene!, stream: X3DOutputStream (style: X3DOutputStyle (style) ?? .Tidy))
      }
      
      public final func toJSONString (_ style : String) -> String
      {
         return executionContext .toJSONString (with: executionContext .scene!, stream: X3DOutputStream (style: X3DOutputStyle (style) ?? .Tidy))
      }

      public final func toVRMLString (_ style : String) -> String
      {
         return executionContext .toVRMLString (with: executionContext .scene!, stream: X3DOutputStream (style: X3DOutputStyle (style) ?? .Tidy))
      }
   }
}
