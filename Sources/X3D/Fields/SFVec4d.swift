   ...
   X3DFieldInterface
{
   ...
   internal final override func toJSONStream (_ stream : X3DOutputStream)
   {
      ...
      stream += String (format: "\(stream .doubleFormat),\(stream .TidySpace)\(stream .doubleFormat),\(stream .TidySpace)\(stream .doubleFormat),\(stream .TidySpace)\(stream .doubleFormat)", x, y, z, w)
      stream += stream .TidySpace
      stream += "]"
   }
   ...
   internal final override func toVRMLStream (_ stream : X3DOutputStream)
   {
      ...
      stream += String (format: "\(stream .doubleFormat) \(stream .doubleFormat) \(stream .doubleFormat) \(stream .doubleFormat)", x, y, z, w)
   }
